import { Circle } from "./BlogCard";

const BlogSkeleton = () => {
  return (
    <div role="status" className="animate-pulse">
      <div className="border-b-2 border-slate-200 p-4 w-screen max-w-screen-sm cursor-pointer">
        <div className="flex">
          <div>
            <svg
              className="w-8 h-8 text-gray-200 mx-2"
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              viewBox="0 0 20 20"
            >
              <path d="M10 0a10 10 0 1 0 10 10A10.011 10.011 0 0 0 10 0Zm0 5a3 3 0 1 1 0 6 3 3 0 0 1 0-6Zm0 13a8.949 8.949 0 0 1-4.951-1.488A3.987 3.987 0 0 1 9 13h2a3.987 3.987 0 0 1 3.951 3.512A8.949 8.949 0 0 1 10 18Z" />
            </svg>
          </div>
          <div className="pl-2 text-sm flex justify-center flex-col">
            <div className="h-4 bg-gray-200 rounded-full mb-2.5 w-20"></div>
          </div>
          <div className="flex flex-col justify-center pl-2">
            <Circle />
          </div>
          <div className="font-light pl-2 text-slate-500 text-sm flex flex-col justify-center">
            <div className="h-4 bg-gray-200 rounded-full mb-2.5  w-20"></div>
          </div>
        </div>
        <div className="text-2xl font-semibold pt-2">
          <div className="h-6 bg-gray-200 rounded-full mb-2.5"></div>
        </div>
        <div className="text-md ">
          <div className="h-2 bg-gray-200 rounded-full mb-2.5"></div>
        </div>
        <div className="text-slate-600 text-sm font-thin pt-4">
          <div className="h-2 bg-gray-200 rounded-full mb-2.5 w-20"></div>
        </div>
      </div>

      <span className="sr-only">Loading...</span>
    </div>
  );
};

export default BlogSkeleton;
